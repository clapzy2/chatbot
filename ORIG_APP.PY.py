...
MODEL_NAME = "qwen2.5:7b-instruct"  # или "qwen2.5:7b-instruct", "gemma2:9b", "llama3.1:8b"
...
def ask():
    try:
        ...
        def extract_query_keywords(query):
            ...
            stop_words = {'это', 'что', 'как', 'так', 'вот', 'там', 'тут', 'его', 'её', 'их'}
...
ПРИМЕРЫ:
Контекст: "Сократ был философом. Он жил в Афинах."
Вопрос: "Кем был Сократ?"
Ответ: "Сократ был философом."

Контекст: "Девочка зажгла одну спичку. Она увидела ёлку."
Вопрос: "Сколько спичек зажгла девочка?"
Ответ: "Одну спичку."

Контекст: "В лесу жил медведь. Он любил мёд."
Вопрос: "Как звали медведя?"
...
ОТВЕТ:"""
        ...
        def validate_answer(answer, docs):
            ...
            # Если ответ уже говорит об отсутствии информации, возвращаем как есть
            if "нет информации" in answer.lower() or "не знаю" in answer.lower():
                return answer, []

            # Извлекаем из ответа ключевые слова
            keywords = extract_keywords(answer)
            # Проверяем, есть ли хотя бы одно ключевое слово в найденных фрагментах
            found = False
            for doc in docs:
                content = doc.page_content.lower()
                if any(kw in content for kw in keywords):
